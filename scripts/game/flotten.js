...
function storage() {
	...
	var elyriumToTransport = parseInt(document.getElementsByName("elyrium")[0].value);
	var population301ToTransport = parseInt(document.getElementsByName("population301")[0].value);
	var population302ToTransport = parseInt(document.getElementsByName("population302")[0].value);
	var population303ToTransport = parseInt(document.getElementsByName("population303")[0].value);
	var population304ToTransport = parseInt(document.getElementsByName("population304")[0].value);
	var population305ToTransport = parseInt(document.getElementsByName("population305")[0].value);
	var population306ToTransport = parseInt(document.getElementsByName("population306")[0].value);
	var population307ToTransport = parseInt(document.getElementsByName("population307")[0].value);
	...
	if (isNaN(elyriumToTransport)) {
		elyriumToTransport = 0;
	}
	
	if (isNaN(population301ToTransport)) {
		population301ToTransport = 0;
	}
	if (isNaN(population302ToTransport)) {
		population302ToTransport = 0;
	}
	if (isNaN(population303ToTransport)) {
		population303ToTransport = 0;
	}
	if (isNaN(population304ToTransport)) {
		population304ToTransport = 0;
	}
	if (isNaN(population305ToTransport)) {
		population305ToTransport = 0;
	}
	if (isNaN(population306ToTransport)) {
		population306ToTransport = 0;
	}
	if (isNaN(population307ToTransport)) {
		population307ToTransport = 0;
	}
	
	
	return data.fleetroom - dataFlyConsumption - metalToTransport - crystalToTransport - deuteriumToTransport - elyriumToTransport - population301ToTransport - population302ToTransport - population303ToTransport - population304ToTransport - population305ToTransport - population306ToTransport - population307ToTransport;
}
...
function refreshFormData() {
	...
	seconds -= minutes * 60;
	$("#duration").html("<span style='cursor: help;' onmouseout='cache();' name='temps_trajet_aller' class='cyan'>" + hours + (":" + dezInt(minutes, 2) + ":" + dezInt(seconds,2) + " h") + "</span>");
	...
	if (dataFlyCargoSpace >= 0) {
		$("#consumption").html("<span style='cursor: help;' onmouseout='cache();' name='carburant_aller' class='cyan'>" + NumberGetHumanReadable(dataFlyConsumption) + "</span> u");
		$("#storage").html("<span name='fret_restant' class='orange'>" + NumberGetHumanReadable(dataFlyCargoSpace) + "</span> u");
	} else {
...
function maxResource(id) {
	...
	var freeCapacity = Math.max(storCap - storCapBis - metalToTransport - crystalToTransport - deuteriumToTransport - elyriumToTransport - population301ToTransport - population302ToTransport - population303ToTransport - population304ToTransport - population305ToTransport - population306ToTransport - population307ToTransport, 0);
	calculateTransportCapacity();
}

function maxResourceBis(id) {
	...
	var storCap = dataFlyCargoSpace;
	var storCapBis = dataFlyConsumption;

	if (id == 'elyrium') {
		thisresource -= dataFlyConsumption;
	}
	var metalToTransport = parseInt(document.getElementsByName("metal")[0].value);
	var crystalToTransport = parseInt(document.getElementsByName("crystal")[0].value);
	var deuteriumToTransport = parseInt(document.getElementsByName("deuterium")[0].value);
	...
	var freeCapacity = Math.max(storCap - metalToTransport - crystalToTransport - deuteriumToTransport - elyriumToTransport - population301ToTransport - population302ToTransport - population303ToTransport - population304ToTransport - population305ToTransport - population306ToTransport - population307ToTransport, 0);
	document.getElementsByName(id)[0].value = Math.min(freeCapacity + thisresourcechosen, thisresource);
	
	calculateTransportCapacityBis();
...
function calculateTransportCapacity() {
	...
	var elyrium = Math.abs(document.getElementsByName("elyrium")[0].value);
	transportCapacity = data.fleetroom - data.consumption - metal - crystal - deuterium - elyrium;
...
function calculateTransportCapacityBis() {
	var metal = Math.abs(document.getElementsByName("metal")[0].value);
	var crystal = Math.abs(document.getElementsByName("crystal")[0].value);
	var deuterium = Math.abs(document.getElementsByName("deuterium")[0].value);
	var elyrium = Math.abs(document.getElementsByName("elyrium")[0].value);
	var population301 = Math.abs(document.getElementsByName("population301")[0].value);
	var population302 = Math.abs(document.getElementsByName("population302")[0].value);
	var population303 = Math.abs(document.getElementsByName("population303")[0].value);
	var population304 = Math.abs(document.getElementsByName("population304")[0].value);
	var population305 = Math.abs(document.getElementsByName("population305")[0].value);
	var population306 = Math.abs(document.getElementsByName("population306")[0].value);
	var population307 = Math.abs(document.getElementsByName("population307")[0].value);
	transportCapacity = dataFlyCargoSpace - dataFlyConsumption - metal - crystal - deuterium - elyrium - population301 - population302 - population303 - population304 - population305 - population306 - population307;
	updateVars();
	return transportCapacity;
}
...
function CheckTarget()
{
	...
	var storCap = dataFlyCargoSpace;
	var storCapBis = dataFlyConsumption;
	var elyrium = getRessource('elyrium');
	var targetGalaxy = document.getElementsByName("galaxy")[0].value;
	var targetSystem = document.getElementsByName("system")[0].value;
	var targetPlanet = document.getElementsByName("planet")[0].value;
	var targetType = document.getElementsByName("type")[0].value;
	var missions = $('input[name="mission"]:checked').val();
	
	$.getJSON('game.php?page=fleetStep1&mode=checkTarget&galaxy='+targetGalaxy+'&system='+targetSystem+'&planet='+targetPlanet+'&planet_type='+targetType+'&lang='+Lang+'&mission='+missions+'&kolo='+kolo, function(data) {
		
		if(storCapBis > elyrium) {
		$('#reponse_ajax1').html("<span class=rouge>Not enough elyrium on planet</span>");
		}
		else if(storCap < 0) {
			$('#reponse_ajax1').html("<span class=rouge>Not enough cargo capacity</span>");
		}
		else if(data == "OK") {
...
function CheckTargetBis()
{
	...
	var fleet_group = document.getElementsByName("fleet_group")[0].value;
	var target_mission = document.getElementsByName("target_mission")[0].value;
	var targetGalaxy = document.getElementsByName("galaxy")[0].value;
	var targetSystem = document.getElementsByName("system")[0].value;
	var targetSpeed = document.getElementsByName("speed")[0].value;
	var targetPlanet = document.getElementsByName("planet")[0].value;
	var targetType = document.getElementsByName("type")[0].value;
	
	$.getJSON('game.php?page=fleetStep1&mode=checkTargetBis&galaxy='+targetGalaxy+'&system='+targetSystem+'&planet='+targetPlanet+'&planet_type='+targetType+'&lang='+Lang+'&kolo='+kolo, function(data) {
		
		if(data == "OK") {
		$('#mission_output').empty();
		$('#reponse_ajax').empty();
		$('#mission_output').load("game.php?page=fleetStep1&mode=ajax&galaxy="+targetGalaxy+"&system="+targetSystem+"&planet="+targetPlanet+"&type="+targetType+'&token='+token+'&fleet_group='+fleet_group+'&target_mission='+target_mission+'&speed='+targetSpeed);
		} else {
		$('#mission_output').empty();
		}
	});
