...
$.getJSON("game.php?page=overview&mode=newmail",function(t){
...
$("#new_email").show();
$("#gotit").addClass('orange');
e.innerHTML=("("+t.message+")");
}else{
$("#new_email").hide();
$("#gotit").removeClass('orange')
}
...
{		
	$.getJSON('game.php?page=Tower&mode=newimagecheck&imageid='+ID, function(data) {
		if(data == "OK") {
		$('#reponse_ajax').empty();
		$('#reponse_ajax').html("<span class=vert>Your image is successfully changed</span>");
		} else {
		$('#reponse_ajax').html("<span class=rouge>"+data+"</span>");
		}
	});
	return false;
}

var TourGestion={modifierNomPlanete:function(o,i){apprise('<b>'+towername2+'"<i>'+o+'</i>".</b><br /><i>'+towername1+'</i>',{maxWidthWindow:450,input:o,widthInput:150},function(e){e&&$.ajax({type:'POST', url: 'game.php?page=overview&mode=rename&name='+e, success: function(response) {
	 location.reload(); 
}}); })}};

function endfacebook(){xhr=$.ajax({type:"POST",url:"game.php?page=overview&mode=endIntro",dataType:"json",global:false,success:function(e){}})}
...
function msgDel(MessID, MessType)
{
	...
	$.get('game.php?page=messages&mode=delonemsg&MsgID='+MessID, function(data) {
		$('#loading').hide();
