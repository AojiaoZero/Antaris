...
class ShowTicketPage extends AbstractPage 
{
	...
	public function show()
	{
		...
		$cmdd			= HTTP::_GP('cmdd', 'open');
		
		if($cmdd == 'close'){
		$ticketResult	= $GLOBALS['DATABASE']->query("SELECT t.*, COUNT(a.ticketID) as answer FROM ".TICKETS." t INNER JOIN ".TICKETS_ANSWER." a USING (ticketID) WHERE t.ownerID = ".$USER['id']." AND t.status = '2' GROUP BY a.ticketID ORDER BY t.ticketID DESC;");
		}else{
		$ticketResult	= $GLOBALS['DATABASE']->query("SELECT t.*, COUNT(a.ticketID) as answer FROM ".TICKETS." t INNER JOIN ".TICKETS_ANSWER." a USING (ticketID) WHERE t.ownerID = ".$USER['id']." AND t.status != '2' GROUP BY a.ticketID ORDER BY t.ticketID DESC;");
		}
	...
	function send() 
	{
		...
		if(empty($ticketID)) {
			...
			$ticketDetail	= $GLOBALS['DATABASE']->query("SELECT status FROM ".TICKETS." WHERE ticketID = ".$ticketID.";");
			$ticketDetail	= $GLOBALS['DATABASE']->fetch_array($ticketDetail);
