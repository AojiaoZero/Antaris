...
class ShowRecordsPage extends AbstractPage
{
	...
	function rpopulation()
	{
		...
		$researchList	= array_fill_keys($reslist['tech'], array());
		$buildList		= array_fill_keys($reslist['build'], array());
		$PopulationList		= array_fill_keys($reslist['population'], array());
		...
		while($recordRow = $GLOBALS['DATABASE']->fetch_array($recordResult)) {
			if (in_array($recordRow['elementID'], $reslist['defense'])) {
				...
				$buildList[$recordRow['elementID']][]		= $recordRow;
			} elseif (in_array($recordRow['elementID'], $reslist['population'])) {
				$buildList[$recordRow['elementID']][]		= $recordRow;
			}
		}
		...
		$this->display('page.records.population.tpl');
	}
	function rdefense()
	{
		global $USER, $PLANET, $LNG, $resource, $CONF, $UNI, $reslist;
		...
		$this->display('page.records.defense.tpl');
	}
	function rtechs()
	{
		global $USER, $PLANET, $LNG, $resource, $CONF, $UNI, $reslist;
		...
		$this->display('page.records.tech.tpl');
	}
	function rbuild()
	{
		global $USER, $PLANET, $LNG, $resource, $CONF, $UNI, $reslist;
		...
		$this->display('page.records.build.tpl');
	}
	...
	function rfleets()
	{
		...
		$this->display('page.records.fleets.tpl');
	}
}
