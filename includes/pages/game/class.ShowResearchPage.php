...
class ShowResearchPage extends AbstractPage
{
	 ...
	 function cancel()
	{
		...
		if($PLANET['id'] == $USER['b_tech_planet'])
		{
			...
			if(isset($costRessources[904])) { $SQL	.= $resource[904]." = ".$resource[904]." + ".$costRessources[904].", "; }
		...
		$this->redirectTo('game.php?page=research&cmdd=physik');
	}
    ...
    function addbuild()
	{
		...
		if($max_build != 0 && $max_build <= $ActualCount)
			return false;
	...
	public function show()
	{
		...
		$this->tplObj->assign_vars(array(
			...
			'IsFullQueue'	=> $max_build == 0 || $max_build == count($TechQueue),
			'Queue'			=> $TechQueue,
			'mode'			=> $mode,
		));
