...
class ShowBattleSimulatorPage extends AbstractPage 
{
	...
	function send()
	{
		...
		foreach($BattleArray as $BattleSlotID => $BattleSlot)
		{
			if(isset($BattleSlot[0]) && (array_sum($BattleSlot[0]) > 0 || $BattleSlotID == 0))
			{
				...
				$avaible_fleets = array(202,203,209,223,219,210,224,204,205,206,207,211,214,215,216,306,307);
				foreach($BattleSlot[0] as $ID => $Count)
				{
					if(!in_array($ID, $avaible_fleets) || $BattleSlot[0][$ID] <= 0)
					{
			...
			if(isset($BattleSlot[1]) && (array_sum($BattleSlot[1]) > 0 || $BattleSlotID == 0))
			{
				...
				{
					if((!in_array($ID, $avaible_fleets) && !in_array($ID, $reslist['defense'])) || $BattleSlot[1][$ID] <= 0)
					{
		...
		foreach(array(901, 902, 903) as $elementID)
		{
		...
		$stealResourceInformations	= sprintf($LNG['bs_derbis_raport'], 
			...
			pretty_number(ceil($debrisTotal / $pricelist[223]['capacity'])), $LNG['tech'][223]
		);
	...
	function show()
	{
		...
		$this->tplObj->assign_vars(array(
			...
			'defensiveList'	=> array(401,402,403,404,405,406,407,408),
		));
