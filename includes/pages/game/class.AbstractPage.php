...
abstract class AbstractPage 
{
	...
	public function getAvataris($id) 
	{
	global $UNI;
	$avatar = '';
	$stats_sql	=	"SELECT avatar FROM ".USERS." WHERE `universe` = '".$UNI."' AND `id` = '".$id."';";
	$query = $GLOBALS['DATABASE']->query($stats_sql);
	 while ($StatRow = $GLOBALS['DATABASE']->fetch_array($query))
      {
	  $avatar = $StatRow['avatar'];
	...
	{
	$diploAlly	= $GLOBALS['DATABASE']->query("SELECT total_rank FROM ".STATPOINTS." WHERE id_owner = ".$id.";");
	$diploRow = $GLOBALS['DATABASE']->fetch_array($diploAlly);
	$diploRow = $diploRow['total_rank'];
	return $diploRow;
	}
	...
	{
	$diploAlly	= $GLOBALS['DATABASE']->query("SELECT ally_name FROM ".ALLIANCE." WHERE id = ".$allianceID.";");
	$diploRow = $GLOBALS['DATABASE']->fetch_array($diploAlly);
	$diploRow = $diploRow['ally_name'];
	return $diploRow;
	}
	...
	public function getAllianceOwner($allianceID)
	{
	$diploAlly	= $GLOBALS['DATABASE']->query("SELECT ally_owner FROM ".ALLIANCE." WHERE id = ".$allianceID.";");
	$diploRow = $GLOBALS['DATABASE']->fetch_array($diploAlly);
	$diploRow = $diploRow['ally_owner'];
	return $diploRow;
	}
	...
	protected function getNavigationData() 
    {
		...
		$this->tplObj->assign_vars(array(	
			...
			'Alliance_tag' 		=> $this->getAllianceTag($USER['ally_id']),
	...
	protected function printMessage($Message, $fullSide = true, $redirect = NULL) {
		...
		if($USER['user_deleted'] == 1 || $USER['banaday'] > TIMESTAMP || $CONF['game_disable'] == 0){
