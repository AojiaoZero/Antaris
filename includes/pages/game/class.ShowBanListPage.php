...
class ShowBanListPage extends AbstractPage 
{
	...
	function show()
	{
		...
		if($mode == 'life'){
		$banResult	= $GLOBALS['DATABASE']->query("SELECT * FROM ".BANNED." WHERE universe = ".$UNI." AND longer = '2147483647' ORDER BY time DESC LIMIT ".(($page - 1) * BANNED_USERS_PER_PAGE).", ".BANNED_USERS_PER_PAGE.";");
		}elseif($mode == 'temp'){
		$banResult	= $GLOBALS['DATABASE']->query("SELECT * FROM ".BANNED." WHERE universe = ".$UNI." AND longer < '2147483647' ORDER BY time DESC LIMIT ".(($page - 1) * BANNED_USERS_PER_PAGE).", ".BANNED_USERS_PER_PAGE.";");
		}else{
		$banResult	= $GLOBALS['DATABASE']->query("SELECT * FROM uni1_message_banned WHERE universe = ".$UNI." AND longer < '2147483647' ORDER BY time DESC LIMIT ".(($page - 1) * BANNED_USERS_PER_PAGE).", ".BANNED_USERS_PER_PAGE.";");	
		}
		$banList	= array();
