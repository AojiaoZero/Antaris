		...
		if (!empty($Message) && !empty($Subject))
		{
			...
			if($Mode == 0 || $Mode == 2) {
				...
				while($UserData = $GLOBALS['DATABASE']->fetch_array($USERS))
				{
					...
					SendSimpleMessage($UserData['id'], $USER['id'], TIMESTAMP, 7, $From, $pmSubject, $sendMessage);
				}
			}
			if($Mode == 1 || $Mode == 2) {
				...
				while($UserData = $GLOBALS['DATABASE']->fetch_array($USERS))
				{			
					$sendMessage = '<div style="margin:20px auto;width:60%;text-align:left;padding:0px;">
					...
					<div style="text-align:left;padding:10px;"><i>'.$Message.'</i>
					</div>
					...
					$userList[$UserData['email']]	= array(
						...
						'body'		=> bbcode(str_replace('{USERNAME}', $UserData['username'], $sendMessage))
					);
				}
