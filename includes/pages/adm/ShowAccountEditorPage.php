...
function ShowAccountEditorPage() 
{
	...
	switch($_GET['edit'])
	{
		case 'resources':
			...
			$elyr       = max(0, round(HTTP::_GP('elyr', 0.0)));
			...
			if ($_POST)
			{
				...
				if ($_POST['add'])
				{
					if (!empty($id)) {
						...
						$SQL .= "`elyrium` = `elyrium` + '".$elyr ."' ";
						$SQL .= "WHERE ";
						$SQL .= "`id` = '". $id ."' AND `universe` = '".$_SESSION['adminuni']."';";
						$GLOBALS['DATABASE']->query($SQL);
						$after 		= array('metal' => ($before['metal'] + $metal), 'crystal' => ($before['crystal'] + $cristal), 'deuterium' => ($before['deuterium'] + $deut), 'elyrium' => ($before['elyrium'] + $elyr));
					}
