...
class GalaxyRows
{
	...
	public function getGalaxyData()
	{
        ...
        while($this->galaxyRow = $GLOBALS['DATABASE']->fetch_array($galaxyResult))
		{
			...
			if ($this->galaxyRow['planet_cloak'] > TIMESTAMP && $this->galaxyRow['id'] != $USER['id'])
			{
                $this->galaxyData[$this->galaxyRow['planet']]	= NULL;
				continue;
			}
	...
	{
		$total		= $this->galaxyRow['der_metal'] + $this->galaxyRow['der_crystal'] + $this->galaxyRow['der_deuterium'];
		if($total == 0) {
			...
			$this->galaxyData[$this->galaxyRow['planet']]['debris']	= array(
				...
				'deuterium'		=> $this->galaxyRow['der_deuterium'],
			);
		}
	}
	...
	protected function getPlanetData()
	{
		$this->galaxyData[$this->galaxyRow['planet']]['planet']	= array(
			...
			'galaxyshow'			=> $this->galaxyRow['galaxy'],
			'systemshow'			=> $this->galaxyRow['system'],
			'id_owner'			=> $this->galaxyRow['id_owner'],
			'planetshow'			=> $this->galaxyRow['planet'],
