...
class MissionCaseAttackPortal extends MissionFunctions
{
...
HTML;
		...
		$avaible_fleets = array(202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220);
		foreach(array_merge($avaible_fleets, $reslist['defense']) as $elementID)
		{
		...
		foreach($userAttack as $userID => $userName)
		{
			...
			if($combatResult['won'] == 'a'){
			$FromSend = sprintf($LNG['sys_mess_tower_battle_won'],$this->getUsername($TargetOwner));
			}elseif($combatResult['won'] == 'r'){
			$FromSend = sprintf($LNG['sys_mess_tower_battle_lost'],$this->getUsername($TargetOwner));
			}else{
			$FromSend = $LNG['sys_mess_tower'];
			}	
			SendSimpleMessage($userID, 0, $this->_fleet['fleet_start_time'], 6, $FromSend, $LNG['sys_mess_attack_report'], $message);
		...
		foreach($userDefend as $userID => $userName)
		{
			...
			if($combatResult['won'] == 'a'){
			$FromSend = sprintf($LNG['sys_mess_tower_battle_won'],$this->getUsername($StartOwner));
			}elseif($combatResult['won'] == 'r'){
			$FromSend = sprintf($LNG['sys_mess_tower_battle_lost'],$this->getUsername($StartOwner));
			}else{
			$FromSend = $LNG['sys_mess_tower'];
			}	
