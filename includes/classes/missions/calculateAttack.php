...
function calculateAttack(&$attackers, &$defenders, $FleetTF, $DefTF)
{
	...
	$ARES 	= $DRES = array('metal' => 0, 'crystal' => 0, 'deuterium' => 0);
	$ROUND	= array();
	$RF		= array();
	...
	$TRES['attacker']	= $ARES['metal'] + $ARES['crystal'] + $ARES['deuterium'];
	...
	for ($ROUNDC = 0; $ROUNDC <= MAX_ATTACK_ROUNDS; $ROUNDC++) 
	{
		...
		foreach ($attackers as $fleetID => $attacker) {
			...
			foreach ($attacker['unit'] as $element => $amount) {
				...
				$thisShield	= $amount * ($pricelist[$element]['cost'][901] + $pricelist[$element]['cost'][902] + $pricelist[$element]['cost'][905]) / 10 * $shieldTech; //coque
	...
	foreach ($defenders as $fleetID => $defender) {
		foreach ($defender['unit'] as $element => $amount) {
			if ($element < 300) {							// flotte defenseur en CDR
				...
				$DRESDefs['crystal'] += $pricelist[$element]['cost'][903] * ($lost - $giveback) ;
			}
		}
	}
	...
	$debDefDeu = ($DRES['deuterium'] * ($FleetTF / 100)) + ($DRESDefs['deuterium'] * ($DefTF / 100));

	return array('won' => $won, 'debris' => array('attacker' => array(901 => $debAttMet, 902 => $debAttCry, 903 => $debAttDeu), 'defender' => array(901 => $debDefMet, 902 => $debDefCry, 903 => $debDefDeu)), 'rw' => $ROUND, 'unitLost' => $totalLost);
}