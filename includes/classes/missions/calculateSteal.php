...
{	
	...
	foreach($attackFleets as $FleetID => $Attacker)
	{
		...
		$SortFleets[$FleetID]	-= $Attacker['fleetDetail']['fleet_resource_elyrium'];
	...
	foreach($SortFleets as $FleetID => $Capacity)
	{
		...
		$sqlQuery .= "`fleet_resource_elyrium` = `fleet_resource_elyrium` + '".($stealResource[$ForthResource] * $slotFactor)."' ";
