...
class MissionCaseSpyPortal extends MissionFunctions
{
	...
	function TargetEvent()
	{
		...
		$SpyPop		= $fleetAmount >= $MinAmount + 7 * SPY_VIEW_FACTOR;
		...
		SendSimpleMessage($this->_fleet['fleet_owner'], 0, $this->_fleet['fleet_start_time'], 0, sprintf($LNG['sys_mess_tower_spy_owner'],$this->getPlanetName($this->_fleet['fleet_end_id']), $this->_fleet['fleet_end_system'], $this->_fleet['fleet_end_planet']), $LNG['sys_mess_spy_report'], $spyRaport);
		...
		if($targetUser['occultation_tech'] >= $senderUser['sensors_tech']){
		SendSimpleMessage($this->_fleet['fleet_target_owner'], 0, $this->_fleet['fleet_start_time'], 0, $LNG['sys_mess_spy_control'], $LNG['sys_mess_spy_activity'], '<div class="conteneur_message"><div style="text-align : justify;">'.$LNG['fleet_spy_1'].' '.$this->getUsername($StartOwner).' <span class="orange">[SGCF]</span> '.$LNG['fleet_spy_2'].' home001 ['.$this->_fleet['fleet_end_system'].':'.$this->_fleet['fleet_end_planet'].']. '.$LNG['fleet_spy_3'].' :
    </div>
    ...
    <div class="citation">
        ...
        '.$LNG['fleet_spy_4'].' Kehbb ['.$this->_fleet['fleet_start_system'].':'.$this->_fleet['fleet_start_planet'].'].
    </div>
    ...
    <div class="explication_utilisateur">
		...
		if ($targetChance >= $spyChance)
		{
			...
			der_deuterium = der_deuterium + ".($fleetAmount * $pricelist[210]['cost'][903] * (Config::get('Fleet_Cdr') / 100))." 
