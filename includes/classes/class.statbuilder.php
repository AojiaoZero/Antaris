...
class statbuilder
{
	...
	private function GetUsersInfosFromDB()
	{
		...
		foreach($reslist['population'] as $Population){
			$select_pops	.= " SUM(p.".$resource[$Population].") as ".$resource[$Population].",";
		}
	...
	private function GetTechnoPoints($USER) 
	{
		...
		foreach($reslist['tech'] as $Techno) 
		{
			...
			$Units			 = $pricelist[$Techno]['cost'][901] + $pricelist[$Techno]['cost'][902] + $pricelist[$Techno]['cost'][903] + $pricelist[$Techno]['cost'][904] + $pricelist[$Techno]['cost'][905];
	...
	private function GetPopuPoints($USER) 
	{
		global $resource, $reslist, $pricelist;
		$PopuCounts = 0;
		$PopuPoints = 0;
		...
		foreach($reslist['population'] as $Population)
		{
			if($USER[$resource[$Population]] == 0) continue;
			
			$Units			 = $pricelist[$Population]['cost'][901] + $pricelist[$Population]['cost'][902] + $pricelist[$Population]['cost'][903] + $pricelist[$Population]['cost'][904] + $pricelist[$Population]['cost'][905];
			$PopuPoints += $Units * $USER[$resource[$Population]];
			
			$PopuCounts	+= $USER[$resource[$Population]];
			
			$this->setRecords($USER['id'], $Population, $USER[$resource[$Population]]);
		}
		return array('count' => $PopuCounts, 'points' => ($PopuPoints / Config::get('stat_settings')));
	}
	...
	private function GetBuildPoints($PLANET) 
	{
		...
		foreach($reslist['build'] as $Build)
		{
			...
			$Units			 = $pricelist[$Build]['cost'][901] + $pricelist[$Build]['cost'][902] + $pricelist[$Build]['cost'][903] + $pricelist[$Build]['cost'][904] + $pricelist[$Build]['cost'][905];
	...
	private function GetDefensePoints($USER) 
	{
		...
		foreach(array_merge($reslist['defense'], $reslist['missile']) as $Defense) {
			...
			$Units			 = $pricelist[$Defense]['cost'][901] + $pricelist[$Defense]['cost'][902] + $pricelist[$Defense]['cost'][903] + $pricelist[$Defense]['cost'][904] + $pricelist[$Defense]['cost'][905];
	...
	private function GetFleetPoints($USER) 
	{
		...
		foreach($reslist['fleet'] as $Fleet) {	
			...
			$Units			 = $pricelist[$Fleet]['cost'][901] + $pricelist[$Fleet]['cost'][902] + $pricelist[$Fleet]['cost'][903] + $pricelist[$Fleet]['cost'][904] + $pricelist[$Fleet]['cost'][905];
	...
	private function SetNewRanks()
	{
		...
		foreach($this->Unis as $Uni)
		{
			...
			$pop			= array();
			$Rank           = 1;
			$RankQry        = $GLOBALS['DATABASE']->query("SELECT s.id_owner FROM ".STATPOINTS." as s, ".USERS." as u WHERE s.universe = '".$Uni."' AND  s.stat_type = '1' AND s.id_owner = u.id ".((Config::get('stat') == 2)?'AND u.authlevel < '.Config::get('stat_level').' ':'')." ORDER BY popu_points DESC;");
			while ($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$pop[$CurUser['id_owner']] = $Rank;
				$Rank++;
			}
			...
			while($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$QryUpdateStats .= "UPDATE ".STATPOINTS." SET tech_rank = '". $tech[$CurUser['id_owner']] ."', build_rank = '". $build[$CurUser['id_owner']] ."', defs_rank = '". $defs[$CurUser['id_owner']] ."', fleet_rank = '". $fleet[$CurUser['id_owner']] ."', popu_rank = '". $pop[$CurUser['id_owner']] ."', total_rank = '". $Rank ."' WHERE stat_type = '1' AND id_owner = '". $CurUser['id_owner'] ."';";
				$Rank++;
			}
			...
			$pop			= array();
			$Rank           = 1;
			$RankQry        = $GLOBALS['DATABASE']->query("SELECT s.id_owner FROM ".STATPOINTS." as s WHERE s.universe = '".$Uni."' AND  s.stat_type = '2' ORDER BY popu_points DESC;");
			while ($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$pop[$CurUser['id_owner']] = $Rank;
				$Rank++;
			...
			while($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$QryUpdateStats .= "UPDATE ".STATPOINTS." SET tech_rank = '". $tech[$CurUser['id_owner']] ."', build_rank = '". $build[$CurUser['id_owner']] ."', defs_rank = '". $defs[$CurUser['id_owner']] ."', fleet_rank = '". $fleet[$CurUser['id_owner']] ."', popu_rank = '". $pop[$CurUser['id_owner']] ."', total_rank = '". $Rank ."' WHERE stat_type = '2' AND id_owner = '". $CurUser['id_owner'] ."';";
				$Rank++;
			}
	...
	final public function MakeStats()
	{
		...
		while($UserData	= $GLOBALS['DATABASE']->fetch_array($TotalData['Users']))
		{
			...
			$PopulationPoints		= $this->GetPopuPoints($UserData);
			...
			$UserPoints[$UserData['id']]['popu']['count'] 	= $PopulationPoints['count'];
			$UserPoints[$UserData['id']]['popu']['points'] 	= $PopulationPoints['points'];
			...
			$UserPoints[$UserData['id']]['total']['count'] 		= $UserPoints[$UserData['id']]['techno']['count']
																...
																+ $UserPoints[$UserData['id']]['popu']['count']
			...
			$UserPoints[$UserData['id']]['total']['points'] 	= $UserPoints[$UserData['id']]['techno']['points']
																...
																+ $UserPoints[$UserData['id']]['popu']['points'] 
			...
			if($UserData['ally_id'] != 0)
			{
				if(!isset($AllyPoints[$UserData['ally_id']]))
				{
					...
					$AllyPoints[$UserData['ally_id']]['popu']['count']	= 0;
					$AllyPoints[$UserData['ally_id']]['popu']['points']	= 0;
				...
				$AllyPoints[$UserData['ally_id']]['popu']['count']	+= $UserPoints[$UserData['id']]['popu']['count'];
				$AllyPoints[$UserData['ally_id']]['popu']['points']	+= $UserPoints[$UserData['id']]['popu']['points'];
			...
			(isset($UserData['old_popu_rank']) ? $UserData['old_popu_rank'] : 0).", ".
			(isset($UserPoints[$UserData['id']]['popu']['points']) ? min($UserPoints[$UserData['id']]['popu']['points'], 1E50) : 0).", ".
			(isset($UserPoints[$UserData['id']]['popu']['count']) ? $UserPoints[$UserData['id']]['popu']['count'] : 0).", ".
		...
		if(count($AllyPoints) != 0)
		{
			...
			while($AllianceData	= $GLOBALS['DATABASE']->fetch_array($TotalData['Alliance']))
			{
				...
				(isset($AllianceData['old_popu_rank']) ? $AllianceData['old_popu_rank'] : 0).", ".
				(isset($AllyPoints[$AllianceData['id']]['popu']['points']) ? min($AllyPoints[$AllianceData['id']]['popu']['points'], 1E50) : 0).", ".
				(isset($AllyPoints[$AllianceData['id']]['popu']['count']) ? $AllyPoints[$AllianceData['id']]['popu']['count'] : 0).", ".
