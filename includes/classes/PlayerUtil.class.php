...
class PlayerUtil {
	...
	$domaine_extrait=preg_replace('!^[a-z0-9._-]+@(.+)$!', '$1', $address);
	...
	static function createPlayer($Universe, $UserName, $UserPass, $UserMail, $UserLang = NULL, $planetNames, $Galaxy = NULL, $System = NULL, $Position = NULL, $planetname = NULL, $authlevel = 0, $UserIP = NULL )
	{
		...
		$planetID	= self::createPlanet($Galaxy, $System, $Position, $Universe, $userID, $planetNames, true, $authlevel);
	...
	static function createPlanet($Galaxy, $System, $Position, $Universe, $PlanetOwnerID, $planetNames = NULL, $HomeWorld = false, $authlevel = 0)
	{
		$CONF	= Config::getAll(NULL, $Universe);
		...
		$Class				= $PlanetData[$Pos]['image'];
		
		if(empty($planetNames))
		{
			if($HomeWorld) {
				$planetNames	= t('fcm_mainplanet');
			} else {
				$planetNames	= t('fcp_colony');
			}
		}
	
		$SQL	= "INSERT INTO ".PLANETS." SET
				   name = '".$GLOBALS['DATABASE']->escape($planetNames)."',
				   universe = ".$Universe.",
